package org.firstinspires.ftc.teamcode.robots.constatns;

import com.acmerobotics.dashboard.config.Config;

@Config
public class UniversalValues {
    // INTAKE
    public static double DROPDOWN_UP = 0;
    public static double DROPDOWN_DOWN = 0.6;
    public static int GLISIERA_INTAKE_FORWARD = 0;
    public static int GLISIERA_INTAKE_DEFAULT = 0;
    public static double INTAKE_POW = 1;
    public  static double INTAKE_MOBIL_POW = 0.7;
    // outtake
    public static int GLISIERA_DEFAULT = 0;
    public static int GLISIERA_LOW = 100;
    public static int GLISIERA_MID = 1000;
    public static int GLISIERA_HIGH = 1500;
    public static double bratJos = 0.1;
    public static double bratSus = 0.85;
    public static double bratIntermediary = 0.15;
    public static double pivotJos = 0.1;
    public static double pivotSus = 0.5;
    public static double pivotIntermediary = 0.93;
    public static double twistDef = 0;
    public static double twistScore = 0.37;
    public static double CLAW_OUTTAKE_CLOSE = 0.7;
    public static double CLAW_OUTTAKE_OPEN = 0;
    public static double CLAW_OPEN = 0;
    public static double CLAW_CLOSED = 0.5;
}
